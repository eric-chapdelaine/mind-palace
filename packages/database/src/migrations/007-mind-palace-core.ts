export const mindPalaceCoreMigration = {
  version: 7,
  name: "mind palace core domain",
  sql: String.raw`
    ALTER TABLE tasks ADD COLUMN description TEXT;
    ALTER TABLE tasks ADD COLUMN kanban_status TEXT NOT NULL DEFAULT 'inbox'
    ...
    ALTER TABLE tasks ADD COLUMN duration_minutes INTEGER NOT NULL DEFAULT 30 CHECK (duration_minutes > 0);
    ALTER TABLE tasks ADD COLUMN splittable INTEGER NOT NULL DEFAULT 0 CHECK (splittable IN (0, 1));
    ALTER TABLE tasks ADD COLUMN min_chunk_minutes INTEGER NOT NULL DEFAULT 30 CHECK (min_chunk_minutes > 0);
    ALTER TABLE tasks ADD COLUMN max_chunk_minutes INTEGER NOT NULL DEFAULT 180 CHECK (max_chunk_minutes >= min_chunk_minutes);
    ALTER TABLE tasks ADD COLUMN earliest_start TEXT;
    ALTER TABLE tasks ADD COLUMN deadline_at TEXT;
    ALTER TABLE tasks ADD COLUMN fixed_start TEXT;
    ALTER TABLE tasks ADD COLUMN fixed_end TEXT;
    ALTER TABLE tasks ADD COLUMN parent_task_id INTEGER REFERENCES tasks(id) ON DELETE SET NULL;
    ALTER TABLE tasks ADD COLUMN automation_enabled INTEGER NOT NULL DEFAULT 0 CHECK (automation_enabled IN (0, 1));
    ...
    INSERT INTO tags (public_id, title, description, created_at, updated_at) VALUES
      ...
      ('mind-palace:llm-eligible', 'llm_eligible', 'Task that may be explicitly sent to an agent.', '2026-09-10T00:00:00.000Z', '2026-09-10T00:00:00.000Z'),
